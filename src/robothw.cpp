...
#include <cstdio>
                                               /*1.0 no reducer and no offset*/   /*no reducer in actuator neither the joint  TODO:add the reducer*/
...
//	joint_eff_state.resize(DoF, 0); joint_vel_state.resize(DoF, 0); joint_pos_state.resize(DoF, 0);
//	act_eff_state.resize(DoF, 0);   act_vel_state.resize(DoF, 0);   act_pos_state.resize(DoF, 0);
//	pos_limit_up.resize(DoF,0); pos_limit_low.resize(DoF,0); vel_limit.resize(DoF,0);
...
//		if (joint_limits_interface::getJointLimits(joint, limits)) has_limits = true;
//		if (joint_limits_interface::getSoftJointLimits(joint, soft_limits)) has_soft_limits = true;
...
//			joint_limits_interface.registerHandle(limits_handle);
//		}
//		else{
//			const joint_limits_interface::PositionJointSaturationHandle sat_handle(pos_handle, limits);
...
//	size_t num = msg->sender - 0xF1;
//	ros::Time currentTime = msg->stamp;
...
//	joint_limits_interface.enforceLimits(period);
//	jnt_to_act_pos.propagate();
//
//	for (size_t i = 0;i < DoF; i++)  Pub_arm_cmd(0x01, i, joint_pos_cmd[i]);
...
void XMrobothw::read(const ros::Time, ros::Duration period)
{
	...
	//for (size_t i = 0;i < DoF; i++)  Pub_arm_state(0x01, i);
	Pub_base_odom(0x03);
	//wait the call back to get data
	hw_callback_queue.callAvailable(ros::WallDuration(1 /  freq / 3));
	//check the stamps , data is updated?
